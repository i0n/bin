...
# Add app to local Nginx config so that it can be accessed through a browser
# Wrapped nginx.conf actions in a method so it can be reused with other parameters for remote servers
def nginx_auto_config(target_file,nginx_new)
  nginx_config = IO.readlines(target_file)
  nginx_last_line = nginx_config.pop
  nginx_config.push nginx_new
...
nginx_auto_config('/usr/local/nginx/conf/nginx.conf', "
   server {
     listen 80;
